...
func onHttpResponseHeaders(ctx wrapper.HttpContext, pluginConfig config.PluginConfig) types.Action {
	if !wrapper.IsResponseFromUpstream() {
		// Response is not coming from the upstream.
		// Check if this is a first-byte-timeout generated by Envoy, and if so, attempt retry.
		if isFirstByteTimeoutResponse() {
			activeProvider := pluginConfig.GetProvider()
			if activeProvider != nil {
				providerConfig := pluginConfig.GetProviderConfig()
				if providerConfig.IsFirstByteTimeoutRetryEnabled() {
					apiTokenInUse := providerConfig.GetApiTokenInUse(ctx)
					apiTokens := providerConfig.GetAvailableApiToken(ctx)
					action := providerConfig.OnFirstByteTimeout(activeProvider, ctx, apiTokenInUse, apiTokens)
...
// isFirstByteTimeoutResponse checks whether the current response is a timeout
// generated by Envoy due to the x-envoy-upstream-rq-first-byte-timeout-ms header.
// When the upstream does not send the first byte within the configured time,
// Envoy locally generates a 408 response with code_details == "response_timeout".
func isFirstByteTimeoutResponse() bool {
	status, err := proxywasm.GetHttpResponseHeader(":status")
	if err != nil || status != "408" {
		return false
	}
	codeDetails, err := proxywasm.GetProperty([]string{"response", "code_details"})
	if err != nil {
		return false
	}
	details := string(codeDetails)
	return details == "response_timeout" || details == "upstream_response_timeout"
}
